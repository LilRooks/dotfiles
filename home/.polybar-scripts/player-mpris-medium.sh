...
[ ! -f "$indexLocation" ] && player="1" || player="$(< "$indexLocation")" # load player index
...
player_name="$(playerctl -l | sed "${player}q;d")"
# Set center icon
...
if [ "$player_status" = "Playing" ]; then
    ...
    echo "%{A4:$script next:}%{A5:$script prev:}No player found$(playerctl -p "$player_name" metadata title 2> /dev/null)%{A}%{A}"
    exit
fi

# Grab metadata, artist and dash depend on artist existing
metadata="$(playerctl -p "$player_name" metadata artist 2> /dev/null)"
[[ -n "$metadata" ]] && metadata="$metadata - "
metadata="%{A4:$script next:}%{A5:$script prev:} $metadata$(playerctl -p "$player_name" metadata title 2> /dev/null)%{A}%{A}"
